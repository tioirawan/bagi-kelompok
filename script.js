...
async function getData() {
  if (!data.length) {
    data = await (await fetch('./murid.json')).json()
  }
...
async function bagiRata(murid, jumlah) {
  ...
  const perempuan = shuffle(shuffle(shuffle(murid.filter(m => m.kelamin == "p"))))
...
function startLoading() {
  document.getElementById("container").innerHTML = "Mengacak..."
}
